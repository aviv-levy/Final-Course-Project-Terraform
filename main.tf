...
# === Grant cluster-admin to an existing IAM role via EKS Access Entries ===

# Create the access entry for the role
...
# Associate the AmazonEKSClusterAdminPolicy (cluster-wide admin)
resource "aws_eks_access_policy_association" "cicd_admin" {
  ...
  access_scope {
    type = "cluster" # cluster-wide scope
  }
